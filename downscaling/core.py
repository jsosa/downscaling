...
def downscaling_mp(dem_hresf, msk_hresf, wsl_lresf, wd_lresf, thr_val, thr_dpt, thr_decay, wd_hresf, gtiff=True):
    ...
    # Selecting water depth > thr_dpt
    wsl_lres_msk = wsl_lres.where(wd_lres > thr_dpt)  # thr_dpt
    wsl_lres_msk = wsl_lres_msk.to_series().to_frame(
    ).dropna().reset_index(level=['x', 'y'])
    wsl_lres_msk['xmin'] = wsl_lres_msk['x'] - thr_val
    wsl_lres_msk['ymin'] = wsl_lres_msk['y'] - thr_val
    wsl_lres_msk['xmax'] = wsl_lres_msk['x'] + thr_val
    wsl_lres_msk['ymax'] = wsl_lres_msk['y'] + thr_val
    ...
    def downscaling_mp_func(output, dem_hres, wd_hres, wsl_lres_msk):
        ...
        for xmin, ymin, xmax, ymax, wsl in zip(wsl_lres_msk['xmin'], wsl_lres_msk['ymin'],
            ...
            wd = wd.where(wd > 0, 0)
            win = wd_hres.sel(x=slice(xmin, xmax), y=slice(ymax, ymin))
            wd_hres.loc[dict(x=x_hres[(x_hres > xmin) & (x_hres < xmax)], y=y_hres[(
                y_hres > ymin) & (y_hres < ymax)])] = (xr.ufuncs.mean(win, wd)).values
    ...
    split_df = [df for g, df in wsl_lres_msk.groupby(
        np.arange(len(wsl_lres_msk)) // (len(wsl_lres_msk)/nproc))]

    # Define an output queue
    output = mp.Queue()

    # Setup a list of processes that we want to run
    processes = [mp.Process(target=downscaling_mp_func, args=(
        output, dem_hres, wd_hres, split_df[i])) for i in range(len(split_df))]
    ...
    xr.Dataset({'wd': wd_hres}).to_netcdf(
        wd_hresf, encoding={'wd': {'zlib': True}})

    # Output GTIFF file
    if gtiff:
        name = os.path.dirname(wd_hresf) + '/' + \
            os.path.basename(wd_hresf).split('.')[0] + '.tif'
        with rio.open(name, 'w',
                      driver='GTiff',
                      height=len(wd_hres.y),
                      width=len(wd_hres.x),
